...
void mat_to_rand_txt(Mat& image, Mat& check,  std::string const& output_name)
{
  ...
  for(int x=0; x<SIZEX; x++)
  {
    ...
    {
      if(check.at<Vec3b>(Point(x,y))[0]==100)
      {
        //sample:
        //std::cout<<"now\n";
        pix.x= x;
        pix.y= y;
        pix.color = image.at<Vec3b>(Point(x,y));
        writer_rand.add(pix);
      }
    }
  }
...
//CHECK_INPUT:
    ...
    SIZEX=capture.get(CAP_PROP_FRAME_WIDTH);
    SIZEY=capture.get(CAP_PROP_FRAME_HEIGHT);
    ...
    for (int i=0; i<samples_amount; i++)
    {
      ...
      */
      ...
      check_pic.at<Vec3b>(Point(not_sampled_yet[n].first,not_sampled_yet[n].second))[0]=100;
    ...
    for (unsigned int i = 0;i <totalFrameNumber; i++) {
      ...
      if(!currentFrame.empty()){
        ...
        mat_to_rand_txt(currentFrame, check_pic, output_name);
      }
    }
	  return 0;
}
